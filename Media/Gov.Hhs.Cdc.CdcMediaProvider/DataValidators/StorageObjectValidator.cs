...
{
    public class StorageObjectValidator : IValidator<StorageObject, StorageObject>
    {
        public void PreSaveValidate(ref ValidationMessages validationMessages, IList<StorageObject> items)
        {
            foreach (StorageObject storageObject in items)
            {
                ...
                v.IsValid(v.Alphanumeric, storageObject.Name, required: true, message: "Name is invalid");
                v.IsValid(v.Alphanumeric, storageObject.FileExtension, required: true, message: "File Extension is invalid");
                v.IsValid(v.Numeric, storageObject.Height.ToString(), required: true, message: "Height is invalid");
                v.IsValid(v.Numeric, storageObject.Width.ToString(), required: true, message: "Width is invalid");                
            }
        }
